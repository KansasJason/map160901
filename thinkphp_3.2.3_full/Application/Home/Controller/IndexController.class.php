...
class IndexController extends Controller {
    ...
    public function check(){
        ...
        $name = $_POST['u_name'];
        $pwd = $_POST['pwd'];
        $id = $User->where(array('username' => $name))->getField('user_id');
        session('id',$id);
        $password =$User->where(array('username' => $name))->getField('password');

        if($id > 0 && $password === $pwd){
            $this->ajaxReturn(aa('',0,'登录成功'));
    ...
    public function back(){
        session(null);
        $this->success('退出成功','Index/indexf');
    }
   public function add(){
        $User = D("User");
       $a = $User->avatar = '';
    ...
    public function search(){
        $User = D("User");
        $name = $_POST['u_name'];
        $url = $User->where(array('username'=> $name))->getField("avatar");
        if($url){
            $this->ajaxReturn(array(
                'data' => $url,
                'status' => 0,
    ...
    public function revise(){
        ...
        if (!$this->checkpwd(I('post.oldpwd'))) {
            ...
            $flg = $User->where(array('user_id'=>$id))->save($data);
            if($flg){
                return $this->ajaxReturn(array(
                    'data'   =>'',
                    'status' => 0,
    ...
    public function checkpwd($data){
        $User = M("User");
        $id = session("id");
        $pwd = $User->where(array('user_id'=>$id))->getField('password');
        if($data != $pwd){
            return false;
        }else{
            return true;
    ...
    public function getimg(){
        $User = D("User");
        $id = session("id");
        $url = $User->where(array('user_id'=>$id))->getField("avatar");
        $name = $User->where(array('user_id'=>$id))->getField("username");
        if($url){
            $this->ajaxReturn(array(
                'data' => $url,
                'status' => 0,
    ...
    public function saveavatar(){
        ...
        $flg = $User->where(array('user_id'=>$id))->save();
        if($flg){
            $this->ajaxReturn(array(
                'data' => '',
                'status' => 0,
